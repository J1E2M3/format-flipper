...
The same engine ships as a Node library and CLI in [`packages/format-flipper`](./packages/format-flipper/):
...
The package's engine code is generated byte-for-byte from `index.html` — the browser tool stays the single source of truth, and CI fails if the two drift. Your data is processed on your own machine, same as the browser tool.
